...
設定は `~/.launcher_tc.json` に自動保存・読み込みされる。
...
```
launcher/
  launcher.py      メイン実装 (約1800行、単一ファイル)
  SPEC.md          本仕様書
...
| `HotlistDialog` | Ctrl+Dホットリストダイアログ |
...
```
#0      アイコン列 (PhotoImage、幅20px、show="tree headings"で表示)
name    名前 (300px、w-anchor、stretch=True)
ext     拡張子 (55px)
size    サイズ (80px、右寄せ)
date    更新日時 (120px)
...
`selectmode="none"` にすると `selected` 状態が設定されず、`s.map()` の色が効かない。
必ず `selectmode="browse"` を使うこと。
...
アイコンは `FilePanel` の `self.icon_folder` / `self.icon_file` に保持。
`_build()` 内で初回のみ生成し、`_populate()` の各行に設定する。
...
テンキーキーはダイアログのEntry内で発火しないよう `_noentry()` ラッパーを使用。
